...
# Compares RNA-seq results to chosen control to identify differentially expressed genes.
# Adapted from the previous iteration called
# rnaseq_two_three.sh
# Compares TWO conditions, each with THREE technical replicates
...
cat >${folder}/twoFactorComparison/${treatment}_over_${control}_deseq/${treatment}_over_${control}_deseq'.R' <<EOF
  ...
  data1 <- read.table("../../counts/${treatment}_${delim}1_featurecounts_Count.txt",header = TRUE,skip=1)
  names(data1)[names(data1) == "aligned.${treatment}_${delim}1_sorted.bam"] <- "${treatment}_${delim}1"
  data2 <- read.table("../../counts/${treatment}_${delim}2_featurecounts_Count.txt",header = TRUE,skip=1)
  ...
  data4 <- read.table("../../counts/${treatment}_${delim}4_featurecounts_Count.txt",header = TRUE,skip=1)
  names(data4)[names(data4) == "aligned.${treatment}_${delim}4_sorted.bam"] <- "${treatment}_${delim}4"
  ...
  more <- res_tableKD_tb %>%
    dplyr::filter(padj <= 0.01 & abs(log2FoldChange) > 0)
  write.table(sigKD,file="${treatment}_${control}_fc0.csv",row.names = FALSE,quote = FALSE,sep = ",")
  ...
  up <- res_tableKD_tb %>%
    ...
    write.table(up,file="${treatment}_${control}_up_p0.01.csv",row.names = FALSE,quote = FALSE,sep = ",")

  down1 <- res_tableKD_tb %>%
    dplyr::filter(padj <= 0.01 & log2FoldChange < 0)
  write.table(down,file="${treatment}_${control}_down_p0.01.csv",row.names = FALSE,quote = FALSE,sep = ",")
...
res_tableKD_tb <- res_tableKD_tb %>% 
	mutate( threshold_KD = case_when(log2FoldChange >= 0 & padj <= 0.01 ~ "Upregulated",
	log2FoldChange <= 0 & padj <= 0.01 ~ "Downregulated",
	TRUE ~ "Unchanged"))

write.csv(res_tableKD_tb, "rawVisual_p0.01.csv")
  ...
  ggplot(res_tableKD_tb) +
    geom_point(aes(x = log2FoldChange, y = -log10(padj), colour = threshold_KD), size = 2, alpha =.7) +
  ...
  ggsave("${treatment}_${control}_VolcanoPlot.pdf",dpi=300)
  ...
  ggsave("${treatment}_${control}_VolcanoPlot_t50.pdf",dpi=300)

  p2 <- ggplot(res_tableKD_tb, aes(log2(baseMean), log2FoldChange)) +
		geom_point(aes(color = threshold_KD), size = 2, alpha = .7) +
  ...
  ggsave("${treatment}_${control}_MA.pdf")
  ...
  p3 <-  p2 +
  	...
  	label = gene), min.segment.length = 0.0000001, size = 2)
  print(p3)
  ggsave("${treatment}_${control}_MA_t50.pdf")

res_tableKD_tb <- res_tableKD_tb %>% 
	mutate( threshold_KD = case_when(log2FoldChange >= 0.58 & padj <= 0.05 ~ "Upregulated",
  ...
  ggsave("${treatment}_${control}_fc1.5_VolcanoPlot_t50.png",dpi=300) 
  
  p2 <- ggplot(res_tableKD_tb, aes(log2(baseMean), log2FoldChange)) +
		geom_point(aes(color = threshold_KD), size = 1, alpha = .7) +
		xlab(expression("log"[2]*"baseMean")) +  
		ylab(expression("log"[2]*"foldChange")) +
		scale_color_manual(values = c("dodgerblue3", "gray50", "firebrick3")) +
		theme_bw() + theme(legend.position = "none")
  ...
  ggsave("${treatment}_${control}_MA_fc1.5.pdf")
  ...
  p3 <-  p2 +
  	...
  	label = gene), min.segment.length = 0.0000001, size = 2)
  print(p3)
  ggsave("${treatment}_${control}_MA_fc1.5_t50.pdf")
...
res_tableKD_tb <- res_tableKD_tb %>% 
  ...
  ggsave("${treatment}_${control}_fc2_MA.pdf")
  ...
  p3 <-  p2 +
  	...
  	label = gene), min.segment.length = 0.0000001, size = 2)
  print(p3)
  ggsave("${treatment}_${control}_MA_fc2_t50.pdf")
EOF
