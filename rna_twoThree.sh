...
# Compares RNA-seq results to chosen control. Adapted from the previous iteration called
# rnaseq_two_three.sh
...
while IFS=$'\t' read -r -a varArray; do
	...
	mkdir -p ${folder}/twoFactorComparison/${treatment}_${control}_deseq
	cat > ${folder}/twoFactorComparison/${treatment}_${control}_deseq/meta.txt<<EOF
    sampletype
...
cat >${folder}/twoFactorComparison/${treatment}_${control}_deseq/${treatment}_${control}_deseq'.R' <<EOF
...
cd ${folder}/twoFactorComparison/${treatment}_${control}_deseq
